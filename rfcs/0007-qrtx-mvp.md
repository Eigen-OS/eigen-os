...
**Device Status Aggregation:** The Kernel is responsible for providing an aggregated device status to the System API. This status combines:
1.  **Raw Backend Status:** Fetched from the Driver Manager (which calls the vendor backend).
2.  **Scheduler State:** Internal queue depth, reservation status, and estimated wait time managed by the Kernel's resource manager.
The aggregated `DeviceStatus` (used by the public `DeviceService`) must reflect both the hardware availability and its logical availability within the Eigen OS scheduling context.
