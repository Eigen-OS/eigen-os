...
**Note on `monitoring.proto` and `auth.proto`:**
The `monitoring.proto` and `auth.proto` files present in the repository structure are **not part of the public gRPC API in MVP**. Their definitions are reserved for future internal use or later API versions.
- **Authentication & Authorization** are enforced by System API interceptors based on HTTP/gRPC metadata headers. No public RPC calls for auth are exposed in MVP.
- **Metrics** are exposed via a separate HTTP endpoint (`/metrics` in Prometheus format) for scraping, not via a public gRPC service.

This decision keeps the public API contract minimal and focused on the core quantum workflow.
...
**MVP Implementation Note:** To simplify the initial Kernel (QRTX) implementation, the `StreamJobUpdates` RPC in MVP is implemented using a **polling-based mechanism**. The System API will periodically poll the Kernel for job updates and forward them to the client. This provides the streaming semantics to the client while allowing the Kernel to maintain a simpler, event-driven internal state model without a full-blown publish-subscribe system. A true event-driven stream from the Kernel may be introduced in Phase 1.
