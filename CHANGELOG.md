...
- VQE result projection ignores non-finite objective values and deterministically keeps the earliest evaluation when multiple observations have the same lowest energy.
...
- Phase-8C P8C-06 adaptive-loop observability + alert pack (`runtime-observability` assets `0.5.0`) with deterministic retrain-queue/promotion-failure/rollback-rate alerts, traffic-aware noise suppression guards, and runbook-linked owner escalation map.
