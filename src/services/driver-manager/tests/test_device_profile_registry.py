...
def test_profile_negotiation_is_fail_closed_on_unknown_profile() -> None:
    registry = DriverRegistry()
    registry.add_driver(
        "qiskit-runtime",
        _FakeDriver(
    ...
    with pytest.raises(
        ValueError,
        match="unsupported device profile 'unsupported-profile'",
    ):
        registry.negotiate_device_profile(
            "ibm:qiskit-runtime",
            requested_profile="unsupported-profile",
        )
