"""Protobuf codegen helper for driver-manager internal API."""
...
def _default_proto_files(proto_root: Path) -> list[Path]:
    return sorted(
        [
            proto_root / "eigen_internal" / "v1" / "types.proto",
            proto_root / "eigen_internal" / "v1" / "driver_manager_service.proto",
        ]
    )
...
def get_paths() -> ProtoGenPaths:
    ...
    out_dir = repo_root / "src" / "services" / "driver-manager" / "src"
...
def ensure_generated(files: Iterable[Path] | None = None) -> None:
    ...
    sentinel = paths.out_dir / "eigen_internal" / "v1" / "driver_manager_service_pb2.py"
    if sentinel.exists():
        return
    ...
    try:
        ...
        raise RuntimeError(
            "grpcio-tools is required to generate protobuf stubs for driver-manager"
        ) from e
