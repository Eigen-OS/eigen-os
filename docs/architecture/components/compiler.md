...
The replay bundle must also preserve the exact advisory snapshot pins used by the compile: model snapshot identifier and digest, knowledge-base snapshot identifier and digest, and policy snapshot version, identifier, and digest. Those values are part of the compiler provenance and are required for deterministic replay.
