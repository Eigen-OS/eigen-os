...
The ML advisor never becomes the source of truth for legality, topology, policy, or device state. The advisor is deployed as a pure sidecar service: compiler correctness MUST remain unchanged if the advisor is restarted, replaced, or disabled.
